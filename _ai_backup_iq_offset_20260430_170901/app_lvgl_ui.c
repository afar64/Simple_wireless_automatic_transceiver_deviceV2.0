...
static lv_obj_t *g_amplitude_slider = NULL;
static lv_obj_t *g_rate_slider = NULL;
static lv_obj_t *g_param_slider = NULL;
static lv_obj_t *g_amplitude_value_label = NULL;
...
void App_LvglUiInit(void)
{
  ...
  App_LvglUiCreateSliderRow(panel,
                            "Amplitude",
                            (int32_t)APP_DAC_WAVE_IQ_AMPLITUDE_MIN_MV,
                            (int32_t)APP_DAC_WAVE_IQ_AMPLITUDE_MAX_MV,
                            (int32_t)status.config.amplitude_mv,
                            142,
                            NULL,
                            &g_amplitude_slider,
                            &g_amplitude_value_label);
  App_LvglUiCreateSliderRow(panel,
                            "Rate",
...
static void App_LvglUiCreateTitle(lv_obj_t *parent)
{
  ...
  lv_label_set_text(subtitle, "DAC1_OUT1 = I / DAC1_OUT2 = Q / 2 MHz sample rate");
...
static void App_LvglUiRefreshLabels(void)
{
  ...
  int32_t amplitude = lv_slider_get_value(g_amplitude_slider);
  int32_t rate = lv_slider_get_value(g_rate_slider);
  int32_t param = lv_slider_get_value(g_param_slider);

  lv_label_set_text_fmt(g_amplitude_value_label, "%ld mV", (long)amplitude);
...
static void App_LvglUiReadConfig(AppDacWavegenConfig *config)
{
  ...
  config->amplitude_mv = (uint16_t)lv_slider_get_value(g_amplitude_slider);
