...
/* USER CODE BEGIN 0 */
/* 下面这些模式寄存器位来自当前板卡已验证可点屏的 SDRAM 配置。 */
...
/* 执行外部 SDRAM 上电初始化序列，并写入当前时钟前提下的刷新率。 */
static void SDRAM_Initialization_Sequence(SDRAM_HandleTypeDef *hsdram)
{
  FMC_SDRAM_CommandTypeDef command = {0};
  uint32_t mode_register = 0;

  /* 先给 SDRAM 时钟，后面的预充电和模式寄存器设置才有意义。 */
  ...
  /* 刷新率必须和当前 FMC 时钟匹配，否则后面可能出现随机花屏或数据损坏。 */
...
void MX_FMC_Init(void)
{
  ...
  */
  ...
  /* HAL_SDRAM_Init 只配置控制器；外部芯片仍需在这里补完整上电命令序列。 */
